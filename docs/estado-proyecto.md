...
- MIGRACION APUNTETEMA APLICADA CONTROLADAMENTE A APRENDIZAJEPERSONALDB CON BACKUP PREVIO
- RUNTIME PERSONAL TEMA WORKSPACE Y APUNTES VALIDADO END-TO-END
...
- TEMA WORKSPACE FRONTEND V1 IMPLEMENTADO SOBRE TEMAWORKSPACEV1
- TEMA WORKSPACE V1 CENTRALIZA OBJETIVOS, CRITERIOS, PROGRESO, APUNTES Y CONTEXTO DEL TEMA
- TEMA WORKSPACE V1 MANTIENE PROGRESO, ESTADO Y REPASO FIELES AL BACKEND
- APUNTES PERMANENTES INTEGRADOS VISUALMENTE Y DIFERENCIADOS DE SESIONES Y RESOURCES
...
- RECURSOTEMA READ CONTRACT V1 BACKEND VALIDADO END-TO-END
- GET /api/recursos SOPORTA FILTRO CONTEXTUAL temaId SIN USUARIOID EN PERSONAL
...
- Ninguna documentada actualmente para AprendizajePersonalDb.
...
- Ruta `/roadmap/tema/:temaId` reemplaza el placeholder por Tema Workspace frontend V1 y consume `GET /api/temas/{temaId}/workspace` sin usuarioId.
- Tema Workspace frontend V1 muestra Tema/Fase, objetivos, criterios de dominio, progreso, EstadoTema, percepcion, ultima sesion, repaso, apuntes permanentes y resumenes de Resources/Sesiones/Evidence.
...
- Tests de EvidenceListaV1: Application valida usuario/filtros, API Personal valida GET /api/evidence sin usuarioId y filtros, e integracion SQL valida lista unificada de Proyecto, Laboratorio, Writeup, ArtefactoTecnico y CertificacionObtenida, EstadoMadurez completo, relaciones por Tema, CertificacionObtenida via CertificacionTema, Herramientas donde aplican, ownership, soft delete, filtros combinados, no regresion de Portafolio y ausencia de N+1 obvio con maximo 13 comandos de lectura.
- Tests de RecursoTema Read Contract V1: Application valida contrato de list/detail con Temas y filtro temaId; API Personal valida GET /api/recursos sin usuarioId, temaId y detail con Temas; integracion SQL valida lista global, recurso sin Temas, uno y multiples Temas, filtro temaId, Tema ajeno como lista vacia, Resource/Tema soft-deleted excluidos, ownership, write PUT idempotente preservado y ausencia de N+1 obvio con maximo 2 comandos de lectura para LIST.
...
- Tests de integracion EvidenceListaV1: validan inclusion de los cinco ARs en lectura, filtros por tipo/EstadoMadurez/TemaId, contexto por Tema, herramientas aplicables y separacion semantica respecto de Portafolio.
